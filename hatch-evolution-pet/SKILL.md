...
description: Create, evolve, repair, validate, preview, and package Codex-compatible animated pet spritesheets with persistent progression rules and machinespace-backed lineage state. Use when a user wants to create an evolving Codex pet, hatch a custom animated pet, check pet progression, respond to evolution readiness, generate an unlocked evolved form, or maintain pet machinespace state. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
---
...
After the base job has been recorded and `references/canonical-base.png` exists, generate row strips through the same `$imagegen` + manifest + QA loop as the original hatch-pet workflow.
...
4. Generate and record `idle` and `running-right` first as identity and gait checks.
5. Inspect those rows against the base and references before continuing.
6. Decide whether `running-left` is safe to derive by mirror; if not, generate it as a normal grounded `$imagegen` row.
7. Generate, inspect, and record every remaining non-derived row job using its row prompt and every listed input image.
8. Run repair queueing as needed, then finalization, QA, and packaging.

For each row job, pass the row prompt and all listed input images to `$imagegen`. Do not draw, tile, edit, or synthesize sprites locally. After selecting a generated candidate, record it with `record_imagegen_result.py`; do not manually edit the manifest or copy files into `decoded/`.

Optional subagent use: If the user explicitly asks for parallel generation or subagents, row-strip visual jobs may be delegated. In that case, the parent still owns `imagegen-jobs.json`, `record_imagegen_result.py`, `derive_running_left_from_running_right.py`, repair queueing, finalization, QA, and packaging. Subagents may only generate candidate images and return the selected original `$CODEX_HOME/generated_images/.../ig_*.png` path plus a brief QA note.
...
- Generate row strips sequentially by default. Use subagents only when the user explicitly asks for parallel generation or subagents.
...
- Generate `running-right` first, then mirror `running-left` only when visual inspection confirms a mirror preserves identity and semantics; otherwise generate `running-left` as a normal grounded `$imagegen` row.
